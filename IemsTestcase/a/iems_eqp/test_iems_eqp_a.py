...
class TestIemsEqpA(unittest.TestCase):
    """测试档案相关"""
    ...
    def test_01_import_gateway(self):
        ...
        with allure.step("登录a2.0"):
            IemsLogin(self.driver).a_login_old(self.login.user[2], self.login.pwd[2])
        with allure.step("选择项目，进入设备档案"):
            IEMSEquipment(self.driver).enter_eqp(self.project.bar_project_name[2])
    ...
    def test_05_edit_bulid(self):
        ...
        # IemsLogin(self.driver).a_login_old(self.login.user[2], self.login.pwd[2])
        IEMSEquipment(self.driver).enter_project_space(self.project.bar_project_name[2])
        # 新增建筑
